...
export let FormComponent = class FormComponent {
    constructor(fb, http) {
        ...
        this.skillList = ['Artificial Intelligence', 'Data Science', 'Python Programming', 'Java', 'Network Security', '.Net'];
    ...
    submit() {
        ...
        if (this.userForm.valid && this.academicForm.valid) {
            ...
            formdata.append('dataToSend', new Blob([JSON.stringify(dataToSend)], { type: 'multipart/form-data' }));
            ...
            // next: (response) => {
            this.http.post(`${this.apiUrl}`, formdata, { headers: headers, observe: 'response' })
                .subscribe(response => {
